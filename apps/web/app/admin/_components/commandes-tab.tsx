...
export function CommandesTab({ jwt }: { jwt: string }) {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      setCommandes(await api.functional.commandes.listAll(authConnection(jwt)));
    } catch {
  ...
  const updateStatus = async (id: string, status: string) => {
    setUpdating(id);
    try {
      await api.functional.commandes.status.updateStatus(authConnection(jwt), id, {
  ...
  if (!commandes && !loading) {
    return (
      <div className="flex flex-col items-center gap-4 py-16">
        <p className="font-playfair text-stone-500">Cliquez pour charger les commandes.</p>
        <button
          onClick={load}
      ...
      {commandes?.map((cmd) => (
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="font-cinzel font-bold text-amber-400">{cmd.customerName}</p>
              <p className="font-mono text-xs text-stone-600">{cmd.id}</p>
              {cmd.promoCode && (
                <p className="font-playfair text-xs text-stone-500">Code : {cmd.promoCode}</p>
              )}
            </div>
          ...
          <div className="flex items-center gap-3">
            <span className="font-playfair text-sm text-stone-400">Statut :</span>
            <select
              value={cmd.status}
