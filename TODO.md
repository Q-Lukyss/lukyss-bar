...
- **Premier admin** : après le premier déploiement, `docker compose -f docker-compose.prod.yml exec -e SEED_ADMIN_PASSWORD='...' api seed` (cf. README "Premier admin en prod") — sinon aucun compte n'existe et personne ne peut se connecter (pas de route d'inscription).
